...
顶上那条 90 秒完整比赛就是这轮之后录的：三回合打满，连招、双方大招、KO、结算全入镜。
...
想自己上手打的话，clone 下来 `godot --path .`：`A/D` 移动、`W` 跳、`J` 轻击、`K` 重击、`L` 格挡、`U` 攒满能量放「一波」。

◇ ◆ ◇

- 上一篇（用 Codex 生图复刻杀戮尖塔）：`lokiwang.com/journal/slay-the-spire-godot-2-hours`
- 引擎：Godot 4.7（GDScript，纯 `_draw()` 程序化矢量美术）
- 音频：MiniMax（`t2a_v2` 语音拟声 + `music_generation` 战斗 BGM）
- 编排：Claude Code · ultracode workflow（Fable 5，12 个 subagent）
- 项目治理：wing（4 条 ADR + combat / rendering / audio 三个领域文档）
