...
platform: weixin
source: mpfb-godot-character-creator
cover: assets/mpfb-godot-character-creator/cover-mp.png
---
...
> 📺 46 秒完整宣传片（滑块、换装、360° 环绕全在里面）在博客原文：`lokiwang.com/journal/mpfb-godot-character-creator`

![Godot 里的实时捏人界面](assets/mpfb-godot-character-creator/promo-poster.jpg)
...
![Blender 里生成的 MPFB 基础人体](assets/mpfb-godot-character-creator/still-blender-preview.png)
...
![Godot 里第一版捏人原型](assets/mpfb-godot-character-creator/still-gender.jpg)

## 03 滑块一拉，人就变

原理其实很朴素：一个参数配两个形态，负向一个正向一个（`slim` / `heavy`），Godot 里 `set_blend_shape_value` 实时插值。

![性别滑块连续变化](assets/mpfb-godot-character-creator/gif-gender.gif)

从女性拉到男性，是连续的，中间任何一帧都能停。

![肌肉与体态](assets/mpfb-godot-character-creator/still-muscle.jpg)

![身高比例](assets/mpfb-godot-character-creator/still-height.jpg)
...
![真实发型：波波头](assets/mpfb-godot-character-creator/still-hair.jpg)

![真实 MakeHuman 系统资产：短发 + 工作套装](assets/mpfb-godot-character-creator/still-worksuit.jpg)
...
![360° 环绕查看](assets/mpfb-godot-character-creator/gif-orbit.gif)
...
![面部形变特写](assets/mpfb-godot-character-creator/face-morph-poster.jpg)

（这段的动态特写也在博客原文的视频里）

肤色是唯一的例外，它没走 Blend Shape，而是在 Godot 里复制一份人体材质，实时改 `StandardMaterial3D.albedo_color`：

![肤色深浅](assets/mpfb-godot-character-creator/still-skin-light.jpg)

![肤色冷暖](assets/mpfb-godot-character-creator/still-skin-warm.jpg)

![双眼大小](assets/mpfb-godot-character-creator/still-eyes.jpg)

![下巴宽度](assets/mpfb-godot-character-creator/still-chin.jpg)

## 09 换装：14 个网格一起变形

发型、胡须、服装、鞋子、纹身，每个槽位都能实时切换或隐藏。

![发型、胡须、服装、鞋子连续切换](assets/mpfb-godot-character-creator/gif-outfit.gif)

关键在于**同名同步**：24 个 Blend Shape 名字在人体和 13 件穿戴物上完全一致，改一个参数，14 个网格一起动。

![胡须：络腮胡](assets/mpfb-godot-character-creator/still-beard.jpg)

![鞋子：运动鞋 / 靴子](assets/mpfb-godot-character-creator/still-shoes.jpg)

![程序生成的可变形纹身](assets/mpfb-godot-character-creator/still-tattoo.jpg)
...
![360° 背面](assets/mpfb-godot-character-creator/still-orbit-back.jpg)
...
![创建属于你的 3D 游戏角色](assets/mpfb-godot-character-creator/still-ending.jpg)
...
- 完整宣传视频与本文原文：`lokiwang.com/journal/mpfb-godot-character-creator`
