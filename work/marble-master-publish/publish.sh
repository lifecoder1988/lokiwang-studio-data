...
# Covers, every body image/GIF, plus both videos.
up "$ASSETS/cover.png"            IMG_cover
up "$ASSETS/cover-mp.png"         IMG_cover-mp
up "$ASSETS/still-machine.jpg"    IMG_still-machine
up "$ASSETS/still-x10.jpg"        IMG_still-x10
up "$ASSETS/still-miss.jpg"       IMG_still-miss
up "$ASSETS/gif-around-panel.gif" IMG_gif-around-panel
up "$ASSETS/gif-gameplay.gif"     IMG_gif-gameplay
up "$ASSETS/gif-reels.gif"        IMG_gif-reels
up "$ASSETS/clip1-gameplay.mp4"   VID_clip1-gameplay
up "$ASSETS/clip2-jackpot.mp4"    VID_clip2-jackpot
...
posters = {"clip1-gameplay": "IMG_still-machine", "clip2-jackpot": "IMG_still-x10"}
...
if [[ -e "$POST_ID_FILE" ]]; then
  ...
  POST_JSON=$("$BLOGCTL" posts create \
    ...
    --tags "deepseek,game-dev,pachinko,ai,web-game" \
    --cover "$COVER" \
